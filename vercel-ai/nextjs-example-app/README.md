...
_NOTE: Retention of detailed timeline information is only available in the latest nightly build that can be run with the docker command above._
...
The approval promise can be completed via the UI as well, select the _approval_ handler. Make sure you address the right workflow (use the same key as you used for kicking off the workflow).
