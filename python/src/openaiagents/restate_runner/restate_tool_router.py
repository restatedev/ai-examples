...
async def restate_tool_router(
    context: RunContextWrapper[EnrichedContext[TCustomContext]],
    req: EmbeddedRequest[TToolInput],
) -> str:
    try:
        tool_response = await context.context["restate_context"].generic_call(
            service=req.agent_name,
        ...
        return e.add_note(
            "The tool could not be called. Make sure the tool_name is correct and the request is valid."
        )
