...
class RestateTool(BaseModel, Generic[I, O]):
    ...
    name: str = Field(
        default_factory=lambda data: getattr(data["tool_call"], "__name__", "")
    )
    description: str = Field(
        default_factory=lambda data: getattr(data["tool_call"], "__doc__", "")
    )
    input_type: Type[I] = Field(
        default_factory=lambda data: get_input_type_from_handler(data["tool_call"])
    )
    output_type: Type[I] = Field(
        default_factory=lambda data: Type[
            getattr(data["tool_call"], "__annotations__", {}).get("return")
        ]
    )
    service_type: ServiceType = Field(
        default_factory=lambda data: get_service_type_from_handler(data["tool_call"])
    )
    ...
    def to_tool_schema(self) -> dict[str, Any]:
        ...
        full_tool_description = (
            f"{self.description}\n\n{VIRTUAL_OBJECT_HANDLER_TOOL_PREFIX}"
            if self.service_type == "VirtualObject"
            else (
                f"{self.description}\n\n{WORKFLOW_HANDLER_TOOL_PREFIX}"
                if self.service_type == "Workflow"
...
async def run(ctx: restate.ObjectContext, req: AgentInput) -> ChatResponse:
    ...
    while True:
        ...
        tool_and_handoff_schemas = {
            format_name(tool.name): tool.to_tool_schema() for tool in agent.tools
        }
        tool_and_handoff_schemas.update(
            {
                format_name(agent_name): agents_dict[
                    format_name(agent_name)
                ].to_tool_schema()
...
async def execute_tool_call(
    ...
    request = RestateRequest[tool_to_call.input_type](
        **json.loads(command_message.arguments)
    )
    ...
    if tool_to_call.service_type == "Service":
        ...
        if request.delay_in_millis > 0:
            ...
            return await ctx.object_call(
                tool_to_call.tool_call, key=request.key, arg=arg
            )
        ...
        TerminalError(
            f"Cannot invoke tool with service type {tool_to_call.service_type}"
        )
