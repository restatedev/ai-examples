...
from utils.context_utils import current_restate_context
...
class DurableModelCalls(MultiProvider):
    ...
    def get_model(self, model_name: str | None) -> Model:
        return RestateModelWrapper(
            super().get_model(model_name or None), model_name, self.max_retries
        )
...
class RestateModelWrapper(Model):
    ...
    def __init__(self, model: Model, model_name: str = "RestateModelWrapper", max_retries: int | None = 3):
        self.model = model
    ...
    async def get_response(self, *args, **kwargs) -> ModelResponse:
        async def call_llm() -> RestateModelResponse:
            with langfuse.start_as_current_observation(
                    ...
                    trace_context=get_trace_context(current_restate_context()),
                    input=kwargs.get("input", None),
                    metadata=args,
            ) as gen:
                # Revert parent span name
                gen.update_trace(name="Agent workflow")
                resp = await self.model.get_response(*args, **kwargs)
                args_dict = dict(enumerate(args)) if args else {}
                gen.update(
                    output={
                        "output": resp.output,
                        "usage": resp.usage,
        ...
        result = await current_restate_context().run_typed(
...
class RestateSession(SessionABC):
    ...
    def _ctx(self) -> restate.ObjectContext:
            ...
            def create_wrapper(captured_tool):
                async def on_invoke_tool_wrapper(
                    ...
                    try:
                        ...
                        return await current_restate_context().run_typed(
...
def get_trace_context(restate_context: restate.Context) -> TraceContext | None:
    trace_parent = restate_context.request().attempt_headers.get('traceparent')
    if trace_parent:
