...
async def run(restate_context: restate.Context, claim: InsuranceClaim) -> str:
    ...
    result = await Runner.run(
        Agent(name="ClaimApprovalAgent", instructions="You are a claim decision engine."),
...
@agent_service.handler()
async def run_eligibility_agent(restate_context: restate.Context, claim: InsuranceClaim) -> str:
    result = await Runner.run(
        Agent(
