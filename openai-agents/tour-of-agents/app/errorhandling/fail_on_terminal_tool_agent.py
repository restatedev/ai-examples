...
weather_agent = Agent[restate.Context](
    name="FailOnTerminalErrorWeatherAgent",
    instructions="You are a helpful agent that provides weather updates. Terminal errors will cause the workflow to fail immediately.",
    tools=[get_weather],
)


agent_service = restate.Service("FailOnTerminalErrorAgent")
