...
async def book_hotel(
    ...
    booking_context.on_rollback.append(lambda: restate_context.run_typed("Cancel hotel", cancel_hotel, booking_id=booking_id))

    # Execute the workflow step
    result = await restate_context.run_typed("Book hotel", reserve_hotel, booking_id=booking_id, booking=booking)
...
async def book_flight(
    ...
    booking_context.on_rollback.append(lambda: restate_context.run_typed("Cancel flight", cancel_flight, booking_id=booking_id))
    result = await restate_context.run_typed("Book flight", reserve_flight, booking_id=booking_id, booking=booking)
    return result.model_dump()
...
async def book(restate_context: restate.Context, message: BookingRequest) -> str:
    ...
    booking_context = BookingContext(booking_id=message.booking_id, restate_context=restate_context)
