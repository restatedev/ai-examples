...
class ModelResponseSerde(Serde[bytes]):
    def deserialize(self, buf: bytes) -> ModelResponse:
            ...
            if item_dict.get("type") == "message":
                ...
                for content_item in item_dict.get("content", []):
                    if content_item.get("type") == "output_text":
                        ...
                        content_list.append(
                            ResponseOutputRefusal(
                                type="refusal", refusal=content_item["refusal"]
                            )
                        )
        ...
        return ModelResponse(
            output=output_items, usage=usage, response_id=data.get("response_id")
        )
    ...
    def serialize(self, obj: typing.Optional[ModelResponse]) -> bytes:
        ...
        return json.dumps(
            {
                "output": [
                    output.model_dump(exclude_unset=True) for output in obj.output
                ],
                "usage": {
...
class RestateModelWrapper(Model):
    ...
    async def get_response(
        ...
        async def call_llm() -> ModelResponse:
            return await self.model.get_response(
